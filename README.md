...
- `src/it_support/store.cljk` — `Store` protocol + `MemStore`:
  registered tickets, committed records, an append-only audit ledger.
- `src/it_support/advisor.cljk` — `Advisor` protocol; `mock-advisor`
...
- `src/it_support/governor.cljk` — `ITSupportGovernor/check`: a pure
...
- `src/it_support/actor.cljk` — `build-graph`, `run-request!`,
