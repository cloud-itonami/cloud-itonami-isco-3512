(ns it-support.actor
  "ITSupportActor — the ISCO-08 3512 independent IT-support actor as a
  `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one support operation request
  ...
  The unconditional invariant: the SupportAdvisor can never directly
  commit a record or dispatch a robot action the ITSupportGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [it-support.advisor :as advisor]
            [it-support.governor :as governor]
            [it-support.store :as store]))

(defn build-graph
  "Build a compiled ITSupportActor graph. `store` implements
  `it-support.store/Store`. `advisor` implements
  `it-support.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:ticket-id (:ticket-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
