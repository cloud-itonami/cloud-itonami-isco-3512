(ns it-support.advisor
  "SupportAdvisor — proposes an IT-support operation (resolve a
  ticket, diagnose a device, reset credentials, change access
  permissions) for a registered ticket. The advisor is swappable:
  ...
  and has no notion of ticket provenance or credential/access-change
  risk; `it-support.governor` is the independent system that decides
  whether the proposal may proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :resolve|:diagnose|:reset-credentials|:change-access-permissions
...
(defn- infer
  ...
  {:op op
   ...
   :rationale (str "proposed " (name op) " for ticket " (:ticket-id request))})
...
(def ^:private system-prompt
  "You are an IT-support advisor. Given a support operation request,
